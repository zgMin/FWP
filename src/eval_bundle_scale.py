...
def gen(model,tok,text,max_new=64):
...
def cb(a,b):
 r=[c for c in a if not c.isspace()]; c=[x for x in b if not x.isspace()]
...
ap=argparse.ArgumentParser(); ap.add_argument("--scale",type=float,required=True); ap.add_argument("--out",required=True); ap.add_argument("--patch",default="/root/zgm/thoughtpatch_qwen25/outputs/thought_patch_attn_o_format_teacher_lm64_scale1.pt"); ap.add_argument("--model",default="/root/zgm/e2pse/models/Qwen2.5-3B-Instruct"); ap.add_argument("--device",default="cuda:0"); ap.add_argument("--eval-limit",type=int,default=0); ap.add_argument("--max-new",type=int,default=64); args=ap.parse_args()
if args.eval_limit>0:EVAL=EVAL[:args.eval_limit]
...
bas=[]
for q in EVAL:
 full=chat(tok,SYSTEM,q); bas.append((q,logits(base,tok,full),gen(base,tok,full,args.max_new)))
bundle=torch.load(args.patch,map_location="cpu",weights_only=False); merge(base,bundle,args.scale)
res=[]
for q,fl,bt in bas:
 qo=chat(tok,"",q); pt=gen(base,tok,qo,args.max_new); ql=logits(base,tok,qo); res.append({"query":q,"kl":kl(fl,ql),"char_bleu":cb(bt,pt),"char_f1":cf(bt,pt),"fmt":fmt(pt),"tokdiff":len(tok.encode(pt,add_special_tokens=False))-len(tok.encode(bt,add_special_tokens=False)),"baseline":bt,"patched":pt})
